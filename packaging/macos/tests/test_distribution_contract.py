...
class DistributionContractTests(unittest.TestCase):
    ...
    def test_runtime_excludes_development_and_build_only_modules(self) -> None:
        spec = self.read("poly-runtime.spec")
        excluded = spec.split("EXCLUDED_IMPORTS = (", 1)[1].split("\n)", 1)[0]

        for module in ("mypy", "mypy_extensions", "setuptools"):
            self.assertIn(f'"{module}",', excluded)
    ...
    def test_runtime_enumeration_can_start_from_a_known_app_pid(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            write_stub(
                ...
                "printf ' 4100 1 %s\\n' \"${POLY_TEST_APP_EXECUTABLE}\"\n"
                "printf ' 4242 4100 poly-runtime\\n'\n"
                "printf ' 9000 1 verifier --enumerate-runtime %s/\\n' \"${POLY_TEST_RUNTIME_ROOT}\"",
            )
            write_stub(
                stubs,
                "lsof",
                ...
                "  *'-p 4100 '*) printf 'p4100\\nn%s\\n' \"${POLY_TEST_APP_EXECUTABLE}\" ;;\n"
                "  *'-p 4242 '*) printf 'p4242\\nn%s/poly-runtime\\n' \"${POLY_TEST_RUNTIME_ROOT}\" ;;\n"
                "  *) printf 'unbounded lsof candidate: %s\\n' \"$*\" >&2; exit 92 ;;\n"
                "esac",
            )
            env = os.environ.copy()
            env["PATH"] = f"{git_bash_path(stubs)}:/usr/bin:/bin"
